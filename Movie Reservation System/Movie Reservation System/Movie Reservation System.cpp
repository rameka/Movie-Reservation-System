/* Movie Reservation System.cpp : Implements the operations of movie reservation system.
*  Author: Ramakrishna Sayee Meka, Syracuse University
*  Date: March 25th, 2018
*  IDE : Visual Studio 2015
*/
//Note: Download MySQL , XAMPP to implement this project 
...
#include<conio.h>
#include<windows.h>
#include <C:\Program Files\MySQL\MySQL Server 5.7\include\mysql.h> // Path to the library is present where MySQL communnity version is installed
#pragma comment(lib, "libmysql.lib") // Copy the specified file 'libmysql.lib' to the directory where .cpp file is present 
using namespace std;

// Global Variable uses for eastablishing the connection with Movie Ticket Booking System database
int qstate;
MYSQL* conn; 
MYSQL_ROW row;
MYSQL_RES* res;
...
public:
	...
	//Note: create the database in XAMPP server. The database file consisting of queries is attached with the project
	static void ConnectionFunction()
	{
		...
		conn = mysql_real_connect(conn, "localhost", "root", "", "moviemanagementdb", 0, NULL, 0);
		if (conn)
		{
			cout << "Connected to database with MySql" << conn << endl;
			cout << "Press any key to continue..." << endl;
			// getchar();
			system("cls");
		}
...
// Fucntionalities of the movie booking system, name of the functions are quite intutive
void BookTicket();
void ShowMyTicket();
void CheckSeat();
void AddNewMovie();
...
int main()
{
	...
	system("cls");
	system("title Movie Booking System"); // Setting the title of the console
	
	//Making the console window Transparent
	HWND hWnd = GetConsoleWindow();
	SetWindowLong(hWnd, GWL_EXSTYLE, GetWindowLong(hWnd, GWL_EXSTYLE) | WS_EX_LAYERED);
	SetLayeredWindowAttributes(hWnd, 0, 220, LWA_ALPHA);
	
	system("color F2"); //setting the color of the system
